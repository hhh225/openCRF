...
void CRFModel::GenFeature()
{
    ...
    // state feature: f(y, x)
    num_attrib_parameter = num_label * num_attrib_type;  //label include 0 and 1,
    num_feature += num_attrib_parameter;//y*x
    ...
    num_feature += num_edge_type * num_edge_feature_each_type;  //228
	...
	for (int i = 0; i < num_triangle_type; i++)
		num_feature += num_triangle_feature[i];  //244
}

void CRFModel::SetupFactorGraphs()
{
    double* p_lambda = lambda + num_attrib_parameter;  //222
...
{   
    ...
    for (int i = 0; i < n; i ++)
    {
        ...
        for (int y = 0; y < num_label; y ++)
        {
            ...
            {
                factor_graph->SetVariableStateFactor(i, y, 0);  //y概率为零
            }
            else
            {
                double v = 1;
                for (int t = 0; t < sample->node[i]->num_attrib; t ++)
                    v *= exp( p_lambda[ sample->node[i]->attrib[t] ] * sample->node[i]->value[t] );          //attrib储存的是特征id
